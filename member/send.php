<?php
	...
	if(isset($_POST['send']))
	{
		...
		try {
			...
			$mail->AddAddress($to);
			$uniqid = $_POST['uniqid'];
			$mail->Subject = "[能量作坊]邮箱帐号激活";
			$yuming = $dir;
			
			$my_url = $yuming.'checkuser.php?uniqid='.$uniqid.'&email='.$_POST['email'];
...
<div style="background:#d0d0d0;padding:40px;text-align:left;">
	<div style="height:420px;width:580px;margin:0 auto;padding:10px;color:#333;background-color:#fff;border:0px solid #aaa;border-radius:5px;-webkit-box-shadow:3px 3px 10px #999;-moz-box-shadow:3px 3px 10px #999;box-shadow:3px 3px 10px #999;font-family:Verdana, sans-serif; ">
		...
		<div>
			...
			$uniqid = strHandle($uniqid);
			$pwd = strHandle($pwd);
			$ip = strHandle($ip);
			
			$query= "insert into zc_user (email,check_str,password,reg_ip,last_ip,reg_time,last_time) value ('$email','$uniqid','$pwd','$ip','$ip',NOW(),NOW())";
