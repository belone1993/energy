<?php
	...
	$newpwd = '';
	for($i=0; $i< 8; $i++){
		$zimu=mt_rand(0, strlen($char)-1);
		$newpwd.=$char[$zimu];
	}
	$email = $_GET['email'];
	$pwd = sha1($newpwd);
	$query = "update zc_user set password='$pwd' where email='$email'";
	if(_mysql($query, 'update')){
	...
	<div style="height:420px;width:580px;margin:0 auto;padding:10px;color:#333;background-color:#fff;border:0px solid #aaa;border-radius:5px;-webkit-box-shadow:3px 3px 10px #999;-moz-box-shadow:3px 3px 10px #999;box-shadow:3px 3px 10px #999;font-family:Verdana, sans-serif; ">
		...
		<div>
			<p class="salutation" style="font-weight:bold;">
			 ...
			 <p>下面为您【能量作坊】帐号的新密码，请妥善保管：<br>
				{$newpwd}<br><br>如果这不是您发送的邮件请忽略，很抱歉打扰您，请原谅。
			</p>
			<div style="padding:40px 0 0;">
				...
				$mail->IsHTML(true);
				$mail->Send();
				echo '<script>alert("新密码已经发送到您的邮箱，请查收");location.href="../login.php";</script>';
		}
		catch (phpmailerException $e) 
		{
				echo '<script>alert("修改失败，请重新点击链接！");window.close();</script>';
		}
	}else{
		echo '<script>alert("修改失败，请重新点击链接！");window.close();</script>';
	}
